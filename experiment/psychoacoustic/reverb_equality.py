...
results_file = slab.ResultsFile(subject='jakab', folder=results_folder)

folder_path = pathlib.Path("/Users/jakabpilaszanovich/Documents/GitHub/distance_perception") / "experiment" / 'samples' / "clap" / "simulated"
...
seq = slab.Trialsequence(conditions=1)
for trial in seq:
    fig_key = None
    ...
    while fig_key != "c":
        ...
        reversed_sound = slab.Binaural.silence(duration=max_length)
        reversed_sound.data[:len(curr_sound)] = curr_sound.data[::-1]
        reversed_sound.play()
